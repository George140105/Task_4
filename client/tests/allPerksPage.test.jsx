...
describe("AllPerks page (Directory)", () => {
  test("lists public perks and responds to name filtering", async () => {
    ...
    // fetch finished. Use findByText which awaits the element to appear.
    ...
    // After typing the full perk name the card should still be present.
    expect(await screen.findByText(seededPerk.title)).toBeInTheDocument();
  ...
  */
  test("lists public perks and responds to merchant filtering", async () => {
    ...
    // Select the merchant from the dropdown. The label isn't programmatically
    // associated with the select element in markup, so query by role instead.
    const merchantSelect = screen.getByRole("combobox");

    // fireEvent.change on the select element to pick the seeded merchant
    fireEvent.change(merchantSelect, {
      target: { value: seededPerk.merchant },
    });

    // Wait for the filtered result to be shown
    expect(await screen.findByText(seededPerk.title)).toBeInTheDocument();

    // Verify the summary text updates and still contains the word 'Showing'
    expect(screen.getByText(/showing/i)).toHaveTextContent("Showing");
  });
});
